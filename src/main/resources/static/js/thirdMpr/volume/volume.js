...
/**
* Created by SongJing on 2018/12/17.
* Version: 图像浏览器 4.0
* Description:
*/
papaya.volume.Volume.prototype.readNextFile = function (vol, index) {
    ...
    if (index < this.files.length) {
     /* blob = papaya.utilities.PlatformUtils.makeSlice(this.files[index], 0, this.files[index].size);
        ...
        blob = this.files[index].data;//其实是arraybuffer
        try {
            vol.rawData[index] = blob;
            setTimeout(function () {vol.readNextFile(vol, index + 1); }, 0);
        } catch (err) {
            vol.error = new Error("There was a problem reading that arraybuffer:\n\n" + err.message);
            vol.finishedLoad();
        }
